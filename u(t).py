...
eigenvalues_raw, eigenvectors_raw = eig(k, m)

# 只取實數根（虛部非常小的視為實數）
mask_real = np.abs(np.imag(eigenvalues_raw)) < 1e-10
eigenvalues = np.real(eigenvalues_raw[mask_real])
eigenvectors = eigenvectors_raw[:, mask_real]

# 按特徵值排序，確保模態對齊
dx = np.argsort(eigenvalues)
eigenvalues = eigenvalues[dx]
eigenvectors = eigenvectors[:, dx]
...
# 激振頻率與阻尼比設定
driver_omega = 6.8 * np.pi  # 激振角頻率 Ω (rad/s)
zeta_list = [0.006, 0.00493, 0.006]  # 三個模態的阻尼比
P0 = 0.0545 * 0.02 * (6.8 * np.pi) ** 2  # 荷載幅值 (N)

# 收集模態向量與 q_n 參數以便組裝位移 u(t)
phi_stack = []
ampl_list = []
phase_list = []
...
for i, x_val in enumerate(eigenvalues):
    ...
    # 收集用於 u(t) 組裝
    phi_stack.append(phi_normalized)
    ampl_list.append(ampl)
    phase_list.append(phi_phase)
...
# 組裝全系統位移 u(t) = Σ φ_n q_n(t)
phi_stack = np.column_stack(phi_stack)
ampl_arr = np.array(ampl_list)
phase_arr = np.array(phase_list)

print("\n" + "="*60)
print("位移組合 u(t) = Σ φ_n * q_n(t)")
print("其中 q_n(t) = A_n * sin(Ω*t - Phi_n), Ω = 6.8π rad/s")
print("="*60)

for dof in range(phi_stack.shape[0]):
    coeffs = phi_stack[dof, :] * ampl_arr
    print(f"\nu_{dof+1}(t) = ", end="")
    terms = []
    for j in range(len(coeffs)):
        terms.append(f"({coeffs[j]:.6e})*sin(Ω*t - {phase_arr[j]:.5f})")
    print(" + ".join(terms))